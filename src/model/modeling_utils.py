...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if "test_mode" in model_kwargs and not model_kwargs['test_mode'] and 'entity_weight' in model_kwargs:
            model.model.shared_entity.weight = torch.nn.Parameter(
                model_kwargs['entity_weight'].type(torch.FloatTensor))
            model.model.encoder.embed_entitys.weight = torch.nn.Parameter(
                model_kwargs['entity_weight'].type(torch.FloatTensor))
            # model.model.decoder.embed_entitys.weight = torch.nn.Parameter(
            #     model_kwargs['entity_weight'].type(torch.FloatTensor))

            assert 'relation_weight' in model_kwargs
            model.model.shared_relation.weight = torch.nn.Parameter(
                model_kwargs['relation_weight'].type(torch.FloatTensor))
