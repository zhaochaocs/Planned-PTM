...
from training.loader_utils import get_random_word, batch_list_to_batch_tensors, Pipeline
import json
...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file_src, file_tgt, batch_size, tokenizer,
        ...
        if pretraining_KG is None:
            ...
            with open(file_src, "r", encoding='utf-8') as f_src, open(file_tgt, "r", encoding='utf-8') as f_tgt:
                ...
                for idx, (src, tgt) in enumerate(zip(f_src, f_tgt)):
                    src_tk = []             # [subword token] e.g., ['ski', 'sk', 'ier', 'mount', 'ain']
                    word_subword = []       # [len of subwords] e.g., [1, 2, 2]
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __call__(self, instance):
        tokens_a, tokens_b, word_subword = instance
        sep_token = "</s>"
        cls_token = "<s>"
        pad_token = "<pad>"
        mask_token = "<mask>"

        tokens_a = tokens_a[:self.max_len_a - 2]
        word_subword = word_subword[:self.max_len_a - 2]
        tokens_b = tokens_b[:self.max_len_b - 2]
        ...
        return (input_ids, subword_mask, word_subword, decoder_input_ids, decoder_attention_mask, labels)
...
class Preprocess4Seq2seqDecoder(Pipeline):
    ...
    def __call__(self, instance):
        tokens_a, word_subword = instance
        sep_token = "</s>"
        cls_token = "<s>"
        pad_token = "<pad>"
        mask_token = "<mask>"
        ...
        return (input_ids, subword_mask, word_subword)
